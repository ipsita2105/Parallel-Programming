...
int get_num_lines(){
	...
	//calucate the line numbers of offsets
...
void* load_file(void* tnum){
	
	char* fname = E.filename;
...
void editorOpen(char* filename){
	...
	clock_t e = clock();
	char ss[20];
	snprintf(ss, sizeof(ss),"time = %f\n", (double)(e-s)/CLOCKS_PER_SEC);
	fwrite(ss, sizeof(char), sizeof(ss), output_open);
	int j;
	...
	pthread_t load_file_threads[NUM_LOAD_FILE_THREADS];	
	
	for(int t=0; t< NUM_LOAD_FILE_THREADS; t++){
		pthread_create(&load_file_threads[t], NULL, load_file, (void *)t);
	}
	...
	fwrite(s2, sizeof(char), sizeof(s2), output_open);
