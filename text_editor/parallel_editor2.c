...
#define NUM_OPEN_FILE_THREADS 4
		...
		while(s->filematch[i]){
			...
			if((is_ext  && ext && !strcmp(ext, s->filematch[i])) ||
			   (!is_ext && strstr(E.filename,  s->filematch[i]))){
				...
				fwrite(s1, sizeof(char), sizeof(s1), output_syntax);
	               		return;
	                }
...
void editorLoadRow(int at, char* s, size_t len){
...
int save_offset[NUM_SAVE_FILE_THREADS];
...
void* rows_to_string(void* tnum){
	...
	char *p = save_buffer + save_offset[my_rank];
...
char* editorRowsToString(int *buflen){
	...
	int linenum[NUM_SAVE_FILE_THREADS];
	for(int i=0; i<NUM_SAVE_FILE_THREADS; i++){
		
		linenum[i] = i*(E.numrows/NUM_SAVE_FILE_THREADS);
	}

	//+1 for newline char
	for(j=0; j< E.numrows; j++){

		for(int p=0; p<NUM_SAVE_FILE_THREADS; p++){
			
			if(linenum[p] == j){
				save_offset[p] = totlen;		
			}
		}
...
int get_num_lines(char *filename){
    ...
    char comm[100];
    sprintf(comm, "wc -l %s > temp.txt", filename);
    system(comm);

    FILE *fp = fopen("temp.txt", "r");
    int nlines;
    char *c;
    fscanf(fp, "%d %s", &nlines, c);

    return nlines;

}

void* process_rows(void *tnum){
	
	int my_rank = (int)tnum;
	int my_start, my_end;

	my_start = my_rank*(E.numrows/NUM_OPEN_FILE_THREADS);

	if(my_rank == NUM_OPEN_FILE_THREADS - 1){
		my_end = E.numrows;
	}else{
		my_end = my_start + (E.numrows/NUM_OPEN_FILE_THREADS);
	}
...
void editorOpen(char* filename){
	...
	E.row = realloc(E.row, sizeof(erow)*(num_lines));
        ...
        while((linelen = getline(&line, &linecap, fp)) != -1){
                ...
                editorLoadRow(E.numrows, line, linelen);
		E.numrows++;
        }
	
	

	pthread_t open_file_threads[NUM_OPEN_FILE_THREADS];

        for(int t=0; t< NUM_OPEN_FILE_THREADS; t++){
                pthread_create(&open_file_threads[t], NULL, process_rows, (void *)t);
        }

        for(int t=0; t< NUM_OPEN_FILE_THREADS; t++){
                pthread_join(open_file_threads[t], NULL);
        }
...
void editorSave(){
	...
	//char *buf = editorRowsToString(&len);
        ...
        fwrite(s, sizeof(char), sizeof(s), output_save);
...
void editorRefreshScreen(){
	...
	fwrite(s2, sizeof(char), sizeof(s2), output_refresh);
...
int main(int argc, char* argv[]){
	...
	output_open = fopen("output_open", "w");
	output_save = fopen("output_save", "w");
	output_syntax = fopen("output_syntax", "w");
	output_refresh = fopen("output_refresh", "w");
	...
	clock_t end = clock();
	char ts[100];
	sprintf(ts, "open file time %f\n",(double)(end-begin)/CLOCKS_PER_SEC);
	fprintf(output_open, "%s",ts);
	...
	fclose(output_open);
	fclose(output_save);
	fclose(output_syntax);
	fclose(output_refresh);
